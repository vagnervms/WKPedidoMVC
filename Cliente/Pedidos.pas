...
begin
  ...
  if Application.MessageBox('Deseja excluir esse item?','Confirmação', MB_YESNO+MB_DEFBUTTON2+MB_ICONQUESTION) = idyes then
  begin
...
begin
  ...
  if Nulos then
  if CDSPedido.Tag <> 0
  then raise Exception.Create('Nemhum parâmetro foi preenchido, busca não pode ser realizada')
  else if Application.MessageBox('Nenhum parâmetro foi preenchido, essa operação poderá levar muito tempo. Deseja continuar?',
                                 'Confirmação',MB_YESNO+MB_DEFBUTTON2+MB_ICONQUESTION) = IDNO then Abort;
